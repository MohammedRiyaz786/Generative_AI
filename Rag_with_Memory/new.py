...
# Load environment variables for SharePoint
load_dotenv()
...
class SharePointReader:
    ...
    def _extract_site_url(self, file_url: str) -> str:
        try:
            if "-my.sharepoint" in file_url:
                match = re.match(r'(https://.*-my\.sharepoint\.com)/personal/[^/]+', file_url)
                if match:
                    return match.group(1)
                raise ValueError("Invalid personal SharePoint URL format")
            elif "sharepoint.com" in file_url:
                match = re.match(r"(https://[^/]+)", file_url)
                if match:
                    return match.group(1)
                raise ValueError("Invalid SharePoint URL format")
            else:
                raise ValueError("Unrecognized SharePoint URL format")
        except Exception as e:
            raise ValueError(f"Error extracting site URL: {str(e)}")

    def _get_relative_url(self, full_url: str) -> str:
        try:
            ...
            if 'id' in query_params:
                relative_path = query_params['id'][0]
                return unquote(relative_path)
            else:
                return unquote(parsed_url.path)
        except Exception as e:
            raise ValueError(f"Error extracting relative URL: {str(e)}")

    def read_file(self, file_url: str):
        try:
            file_url = file_url.replace('%20', ' ')
            site_url = self._extract_site_url(file_url)
            ...
            relative_url = self._get_relative_url(file_url)
            ...
            file_stream.name = os.path.basename(file_url)
            return file_stream
        except Exception as e:
...
def process_sharepoint_file(file_url):
    """Process file from SharePoint URL"""
    try:
        sp_reader = SharePointReader()
        file_content = sp_reader.read_file(file_url)
        
        # Get file extension
        file_extension = os.path.splitext(file_url)[1].lower()
        return extract_file_content(file_content)
    except Exception as e:
        logging.error(f"Error processing SharePoint file: {str(e)}")
        st.error(f"Error processing SharePoint file: {str(e)}")
        return "", []
...
def main():
    try:
        ...
        with st.sidebar:
            ...
            tab1, tab2 = st.tabs(["File Upload", "SharePoint Link"])
            ...
            with tab2:
                ...
                sharepoint_url = st.text_input("SharePoint File URL")
                st.info("Only .pdf, .csv, .xlsx, .pptx, .docx files are supported from SharePoint")
            
            # Process button
            if st.button("Process Documents"):
                if uploaded_files or (tab2 and sharepoint_url):
                    with st.spinner("Processing documents..."):
                        ...
                        if sharepoint_url:
                            try:
                                sp_reader = SharePointReader()
                                file_content = sp_reader.read_file(sharepoint_url)
                                text, docs = extract_file_content(file_content)
                                if text and docs:
                                    all_text += text + "\n\n"
                                    all_docs.extend(docs)
                            except Exception as e:
                        ...
                        if success:
                            ...
                            st.error("Error processing documents. Please check the file format and try again.")
                else:
                    st.warning("Please upload files or provide a SharePoint link before processing.")
        ...
        # Chat interface
        if not st.session_state.docs_processed:
            st.info("👋 Upload your documents or paste a SharePoint link to start chatting!")
