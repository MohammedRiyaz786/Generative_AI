    ...
    try:
        ...
        if doc_hash in faiss_cache:
            ...
            if not faiss_index:
                ...
                combined_text, documents, metadata_chunks = await process_pdf_document(file_content)
...
#                 combined_text, documents, metadata = await process_pdf_document(file_content)
#                 if not combined_text:
#                     raise HTTPException(
#                         status_code=400,
#                         detail="Could not extract text from PDF"
...
#                 # Create vector store
#                 faiss_index, texts = await get_vector_store(documents,metadata)
