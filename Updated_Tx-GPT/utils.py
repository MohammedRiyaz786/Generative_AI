...
def get_ppt_text(ppt_files):
    ...
    for ppt_file in ppt_files:
        try:
            # Try to open as PPTX first
            prs = Presentation(ppt_file)
            
        except Exception as e:
            if "File is not a zip file" in str(e):
                # For legacy .ppt files, we'll need to inform the user
                error_msg = f"Legacy PowerPoint (.ppt) files are not supported. Please save your file as .pptx format."
                st.error(error_msg)
                continue
            else:
        ...
        for slide_num, slide in enumerate(prs.slides):
            ...
            documents.append(Document(
                ...
                metadata={'source': f"PowerPoint Slide {slide_num + 1}"}
            ))
